...
#       --storage-root /mnt/1tb-1/tesl-panel
#
# ВАЖНО: --storage-root должен быть СВОЕЙ выделенной подпапкой, НЕ корнем
# смонтированного диска/раздела целиком — этот путь становится ЕДИНСТВЕННЫМ
# пространством имён приложения (builds_db.py кладёт сюда _meta/builds.db,
# reports_storage.py — _reports/, storage.py — по папке на каждую сборку,
# прямо ТУТ, без общей обёртки) — если диск общий с другими проектами
# (как исходный пример `/mnt/1tb-1` ниже одно время предлагал буквально),
# всё это окажется вперемешку с их файлами в одном каталоге. Живой случай,
# 2026-09-24: реальный деплой так и сделал (буквально по этому же примеру),
# получив _meta/_reports/<сборка> в одном каталоге с чужими
# backup_launcher/cloud/dowsky-files/dowsky-history — см. CLAUDE.md за
# то, как это разобрали на уже развёрнутом сервере, ничего в самом
# скрипте это не чинит автоматически (миграция уже сохранённых данных —
# ручная операция, safety-критичная, делать её отсюда без exec-доступа
# к продакшену тоже нельзя).
#
