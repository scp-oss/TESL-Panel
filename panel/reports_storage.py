...
#
# "manager_log"/"manager_crash" (2026-09-23) — прямой запрос пользователя
# ("добавь дебаг режим чтобы при нём лог приложения отправлялся в панель
# и добавь отправку крашей"), но это ДРУГОЙ клиент: TESL-Manager
# (оператор/дев-инструмент публикации), не лаунчер конечного игрока —
# намеренно отдельные типы, не переиспользуют "crash"/"debug_log", чтобы
# не смешивать в одном разделе крэши реальных игроков Skyrim с
# отладочными логами самого TESL-Manager. Тот же эндпоинт
# (`PUT /api/reports/<type>/...`) и та же раскладка на диске обслуживают
# оба клиента без единой правки здесь — см. TESL-Manager's CLAUDE.md
# ("Дебаг-режим + отправка крашей в панель") за клиентскую сторону.
REPORT_TYPES = {
    "crash":         "Крэш-репорты (Skyrim)",
    "debug_log":     "Логи отладки лаунчера",
    "manager_log":   "Логи TESL-Manager (дебаг-режим)",
    "manager_crash": "Крэши TESL-Manager",
}

# Подпись источника для /admin/reports — "Отчётов от клиентов (лаунчеров)"
# верно только для "crash"/"debug_log"; manager_log/manager_crash приходят
# от совсем другого клиента (оператор/дев-инструмент публикации, не
# конечный игрок), одна и та же подпись под всеми карточками была бы
# фактически неверной для двух из четырёх типов.
REPORT_SOURCES = {
    "crash":         "Отчётов от клиентов (лаунчеров)",
    "debug_log":     "Отчётов от клиентов (лаунчеров)",
    "manager_log":   "Отчётов от TESL-Manager (оператор/дев-инструмент)",
    "manager_crash": "Отчётов от TESL-Manager (оператор/дев-инструмент)",
}
...
# Прямой запрос (2026-09-24): "добавь просмоторщик в панель [для отчётов],
# оставь возможность скачать" — /admin/reports/.../<filename>/view в app.py
# читает файл целиком в память только до этого предела, тот же принцип,
# что storage.py::MAX_INLINE_EDIT_BYTES уже применяет к депо-файлам (там
# 256KB — гораздо меньше, файлы депо там правятся построчно и вручную;
# здесь ЧИТАЕМ, не правим, а реальные краш-логи/Papyrus-логи легко бывают
# по несколько МБ, поэтому предел выше). Выше предела — просмотр
# отказывает с понятным сообщением, кнопка "Скачать" остаётся рабочей
# независимо от размера (см. admin_reports_download — там предела нет).
